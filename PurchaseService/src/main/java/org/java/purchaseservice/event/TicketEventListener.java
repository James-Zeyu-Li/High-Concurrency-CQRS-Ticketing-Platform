...
public class TicketEventListener {
    ...
    public void handleTicketCreation(TicketCreatedEvent ticketCreatedEvent) {
        ...
        try {
            log.info(
                    "【EventListener】Processing TicketCreatedEvent: ticketId={}",
            ...
            if (sent) {
                log.info(
                        "【EventListener】Event sent to Kafka successfully: ticketId={}",
                        ticketCreatedEvent.getTicketId());
            } else {
                log.error(
                        "【EventListener】Failed to send event to Kafka: ticketId={}",
                        ticketCreatedEvent.getTicketId());
                deadLetterQueueService.sendToDeadLetterQueue(payload,
                        partitionKey, "Kafka kafkaPublish returned false");
            }

        } catch (Exception e) {
            log.error(
                    "【EventListener】Error processing TicketCreatedEvent: ticketId={}",
                    ticketCreatedEvent.getTicketId(), e);

            if (payload != null) {
                deadLetterQueueService.sendToDeadLetterQueue(payload,
                        partitionKey, "Exception: " + e.getMessage());
            }
        }
