...
public class KafkaStreamPublisher implements MessagePublisher {
    ...
    @Override
    public boolean kafkaPublish(final String payload, final String partitionKey) {
        try {
            log.info(
                    "【KafkaPublisher】Preparing to send message. binding={}, partitionKey={}, payloadLength={}",
            ...
            if (sent) {
                log.info(
                        "【KafkaPublisher】Message sent to Kafka successfully. partitionKey={}",
                        partitionKey);
            } else {
                log.error(
                        "【KafkaPublisher】Failed to send message to Kafka. partitionKey={}",
                        partitionKey);
            }
            ...
            log.error(
                    "【KafkaPublisher】Exception occurred while sending message to Kafka. partitionKey={}",
                    partitionKey, e);
            return false;
        }
    }
