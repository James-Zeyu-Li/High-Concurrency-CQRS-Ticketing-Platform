...
/**
 * Initializes event configurations during application startup.
 * Runs after VenueConfigService (@Order(2))
 */
@Slf4j
@Component
...
public class EventConfigService implements ApplicationRunner {
    ...
    /**
     * Initializes events based on configuration.
     * Skips if auto-initialize is disabled.
     */
    @Override
    public void run(ApplicationArguments args) {
        ...
        if (!eventConfig.isAutoInitialize()) {
            log.info(
                    "[EventConfigService] Auto-initialize is disabled, skipping event initialization");
            return;
        }
        ...
        if (CollectionUtils.isEmpty(events)) {
            log.warn(
                    "[EventConfigService] No events configured for initialization");
            return;
        }
            ...
            if (event.isEnabled()) {
                try {
                    ...
                    log.info(
                            ...
                            event.getEventId(), event.getVenueId(),
                            initializedCount + 1, totalEvents);

                    // Initiate Redis Zones, bitmap, counters, metadata
                    seatService.initializeAllZonesForEvent(event.getEventId(),
                            event.getVenueId());
                    ...
                    log.info(
                            "[EventConfigService] Successfully initialized event: {}",
                            event.getEventId());
                } catch (Exception e) {
                    // Error handling
                    log.error(
                ...
                // Skip disabled event
                log.info("[EventConfigService] Skipping disabled event: {}",
                        event.getEventId());
            }
        }
