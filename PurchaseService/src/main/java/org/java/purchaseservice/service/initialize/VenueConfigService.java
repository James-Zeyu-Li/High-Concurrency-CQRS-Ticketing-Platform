...
public class VenueConfigService implements InitializingBean {
        ...
        if (!CollectionUtils.isEmpty(venueMap)) {
            ...
            for (Map.Entry<String, VenueConfig.Venue> entry : venueMap.entrySet()) {
                ...
                try {
                    ...
                    venueCount++;
                    log.debug(
                            "[VenueConfigService] Successfully initialized venue: {}",
                            venueId);
                } catch (Exception e) {
                    log.error(
     ...
     */
    public void initializeVenueZone(String venueId, int zoneId, int rowCount,
                                    int colCount) {
        ...
        redisTemplate.opsForValue()
                .set(seatPerRowKey, colCount);  // Number of seats per row

        // get the capacity for zone
        String capacityKey = RedisKeyUtil.getZoneCapacityKey(venueId, zoneId);
        redisTemplate.opsForValue()
                .set(capacityKey, rowCount * colCount);  // Total zone capacity
