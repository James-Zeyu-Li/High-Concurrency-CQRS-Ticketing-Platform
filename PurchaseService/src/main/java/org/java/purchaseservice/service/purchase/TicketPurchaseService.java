...
// Service: Generate ticketId/timestamp
// → Redis seat lock →
// Publish event to Kafka (event-sourced CQRS)
@Service
@Slf4j
...
public class TicketPurchaseService implements TicketPurchaseServiceInterface {
    ...
    public TicketRespondDTO purchaseTicket(TicketPurchaseRequestDTO dto) {
        log.info(
                ...
                dto.getEventId(), dto.getZoneId(), dto.getRow(),
                dto.getColumn());

        // Part 1: Redis - Set Redis seat occupancy to a True - Lua script
        try {
            seatOccupiedRedisFacade.tryOccupySeat(dto.getEventId(),
                    dto.getVenueId(), dto.getZoneId(), dto.getRow(),
                    dto.getColumn());
            log.debug(
        ...
        try {
            ...
            TicketCreationDTO creation =
                    TicketCreationDTO.builder().ticketId(ticketId)
                            .venueId(dto.getVenueId()).eventId(dto.getEventId())
                            .zoneId(dto.getZoneId()).row(dto.getRow())
                            .column(dto.getColumn()).status(TicketStatus.PAID)
                            .createdOn(now).build();

            // Build entity for event publishing
            // (CQRS: event is the source of truth)
            TicketInfo entity = ticketMapper.toEntity(creation);
            entity.setTicketId(ticketId);
            entity.setStatus(creation.getStatus());
            entity.setCreatedOn(now);

            // -- Part 4: Publish Spring Event to Kafka
            // (event-sourced architecture)
            TicketCreatedEvent event =
                    TicketCreatedEvent.builder().ticketId(ticketId)
                            .venueId(dto.getVenueId()).eventId(dto.getEventId())
                            .zoneId(dto.getZoneId()).row(dto.getRow())
                            .column(dto.getColumn())
                            .status(creation.getStatus()).createdOn(now)
                            .build();
    ...
    private void safeReleaseSeat(final TicketPurchaseRequestDTO dto,
        ...
        try {
            seatOccupiedRedisFacade.releaseSeat(dto.getEventId(),
                    dto.getVenueId(), dto.getZoneId(), dto.getRow(),
                    dto.getColumn());
            log.info(
                    "[TicketPurchaseService] seat released after failure, ticketId={}",
                    ticketId);
        } catch (Exception re) {
            log.error(
