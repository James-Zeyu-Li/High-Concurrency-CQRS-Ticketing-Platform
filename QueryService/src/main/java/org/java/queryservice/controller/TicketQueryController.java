...
public class TicketQueryController {
  ...
  public ResponseEntity<String> revenueByEvent(@PathVariable("venueId") String venueId,
      @PathVariable("eventId") String eventId) {
    BigDecimal revenue = queryService.sumRevenueByVenueAndEvent(venueId, eventId);
    String message = String.format("Tickets sold for event %s in venue %s generated revenue %s",
  ...
  public ResponseEntity<Map<String, Object>> getRootInfo() {
    ...
    response.put("availableEndpoints",
        new String[]{"GET /api/v1/health", "GET /api/v1/tickets/{ticketId}",
            "GET /api/v1/tickets/tickets", "GET /api/v1/tickets/count/{eventId}",
            "GET /api/v1/tickets/revenue/{venueId}/{eventId}"});
    response.put("message",
        "QueryService is running. Use specific endpoints for ticket operations.");

    return ResponseEntity.ok(response);
  }

  /**
   * Get all zones according to the event
   */
